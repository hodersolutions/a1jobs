...
class Teachers extends Component {
  ...
  onSubmit({formData}) {
  	...
  	this._asyncRequest = fetch("http://a1api.herokuapp.com/api/v1/users/filter?"+this.state.paramValue+"="+id)
		.then((response) => 
			{
			...
			promise.then(
			  ...
			  console.log(result)
			    if(result.status === "success"){
			    	...
			    	this.setState({teachers: result.object})
			    	console.log(result);
					}
			    }
			)
	...
	render() {
		...
		{
		html_text = 'No Profiles found with the selected criteria'
		}
		for(var i = 0; i < this.state.teachers.length; i++)
		  {
		  	var id = this.state.teachers[i]['Id']
		  	var title = this.state.teachers[i]['fullname']
